  ...
  public List<Instance> findHosts(
    ...
    for (Instance instance: hosts) {
      ...
      if (!instanceHealthMonitor.isHostHealthy(instance, partitionName)) {
        if (storeName == null) {
          /**
           * It seems not clean to use the following method to extract store name, but inside Venice, Kafka topic name
           * is same as Helix resource name. Furthermore, it would require refactoring Alpini APIs to pass the store
           * name up to here.
           */
          storeName = Version.parseStoreFromKafkaTopicName(resourceName);
        }
